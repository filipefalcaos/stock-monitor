...
<template>
  <b-table class="base-text" :data="stockData" :loading="isProcessing" :striped="true" :hoverable="true" :mobile-cards="true">
    <template slot-scope="props">
      ...
      <b-table-column :class="{ 'green-success': newData }" field="current_price" label="Preço Atual" :numeric="true">
        R$ {{ props.row.current_price }}
      </b-table-column>
      <b-table-column :class="{ 'green-success': newData }" field="var" label="Variação" :numeric="true">
        R$ {{ props.row.var }} ({{ props.row.varpct}}%)
        <span v-if="props.row.var > 0">📈</span>
        <span v-else-if="props.row.var < 0">📉</span>
      </b-table-column>
      <b-table-column :class="{ 'green-success': newData }" field="result" label="Resultado" :numeric="true">
        R$ {{ props.row.result }}
